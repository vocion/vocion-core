...
- [Evals: who grades your agent](./guides/evals.md) — **start here.** The two graders side by side with worked YAML, what AWS is actually doing while a job runs, where to look in the console to see it, what lands in which database table, and the setup in order.
