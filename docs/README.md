...
- [Prompt caching](./guides/prompt-caching.md) — the prefix of every agent turn is cached at the vendor by default: what that saves (a cache read is 10% of the input rate **and** does not count toward the Bedrock tokens-per-day quota), the per-model minimum below which a prompt silently does not cache, how to read cache counts in Langfuse and CloudWatch, and the three ways to turn it off.
