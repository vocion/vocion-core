...
The MCP `context_write_*` tools + the in-app Primitive Editor both honor `CONTEXT_PATH`. They write to whatever directory you've pointed at, auto-commit with `chore(context): <summary>` in that repo, and call `context:apply` to sync the DB.
