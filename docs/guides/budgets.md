...
- **During a turn:** every model call is charged as it finishes, and the cap is
  read again straight after. The first call that takes the agent over its cap
  ends the turn — the call in flight is cancelled and no further one starts. A
  turn can go over by at most the one model call that crossed the line.
