...
| **harness** | The machinery that actually runs an agent turn. `harness.provider` on an agent picks where that happens: `local` (in this app's process), `agentcore` (AWS's managed harness runs the loop and calls back for tools), `runtime` (our own loop, in the out-of-process `packages/agent-runtime` artifact — AgentCore Runtime when deployed). The last two are both AWS AgentCore services; they differ in who owns the loop. Leave it out and it is chosen for you: an agent on Bedrock gets `runtime`, everything else gets `local`. |
