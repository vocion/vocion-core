...
| `promptCache` | boolean | on | Ask the vendor to cache this agent's prompt prefix. Written only to say `false` — an agent whose prompt must not sit in a vendor's cache. Beats the call site; `VOCION_PROMPT_CACHE=0` turns it off everywhere. See [prompt caching](../guides/prompt-caching.md). |
