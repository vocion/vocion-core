 ...
 * pass it in CORECONTEXT_PLUGINS. Uses .mjs so Node loads it as ESM without
 * needing tsx/config.
 * @param dir
 * @param name
 * @param contents
...
`);
      ...
      const result = await loadPlugins({ orgId: 'test', env: { CORECONTEXT_PLUGINS: path } });
...
`);
      ...
      const result = await loadPlugins({ orgId: 'org_xyz', env: { CORECONTEXT_PLUGINS: path } });
...
`);
      ...
      const result = await loadPlugins({ orgId: 'test', env: { CORECONTEXT_PLUGINS: [good, bad].join(',') } });
  ...
  it('returns empty when no plugins configured', async () => {
    const result = await loadPlugins({ orgId: 'test', env: { CORECONTEXT_PLUGINS: '' } });
