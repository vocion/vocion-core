...
export default async function FoundationPage(props: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="p-6">
      <TitleBar
        title="Foundation"
        description="The business-context layer that grounds every AI output. Prevents the system from operating on vague or conflicting context."
      />
      ...
      <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <Stat icon={Layers} label="Active agents" value={agents.length} href="/dashboard/ziggy" />
        <Stat icon={CheckCircle2} label="Active skills" value={skills.length} href="/dashboard/skills" />
        <Stat icon={Share2} label="Active workflows" value={workflows.length} href="/dashboard/runs" />
      </div>
