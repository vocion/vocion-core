 ...
 *     Two credential sources: the invocation, when core sent a session
 *     minted from the org's own AWS key, which bills the customer; or
 *     this process's own chain (execution role, or
 *     `AWS_BEARER_TOKEN_BEDROCK`), which bills the platform.
 *
...
/**
 * The credential shape AWS SDK clients accept.
 *
 * Declared structurally rather than imported from `@aws-sdk/types`: that
 * package is a transitive dependency here, not a declared one, and this
 * artifact is bundled on its own.
 */
 ...
 * client wants. Returns undefined to leave the client on its own chain.
 *
 * Returns a **function**, not a credential object. The model is built once per
 * agent definition and cached across invocations, while the session it signs
 * with is per invocation. Reading through a getter on every call means the
 * cached client always signs with the credential of the request being served,
 * and never holds the previous one — the caching mistake that would let one
 * tenant spend another tenant's AWS account.
...
export async function buildChatModel(opts: {
  ...
  maxTokens?: number;
  /**
   * Reads the AWS session of the invocation currently being served.
   *
   * Called on every model request, not once at construction — see
