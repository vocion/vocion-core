...
/**
 * One data room — the collection around one entity, as a page. In reading
 * order: the status, the rules and notes (the room's own knowledge, editable
 * here), the timeline, the sources by weight with who filed each and how
 * (an automatic filing shows its score, and Remove undoes it), the open
 * items, the highlights kept for the case study, the decision logs, the
 * documents written from the room, and the working files. The rail opens
 * beside it scoped to the room; "Download context" is the same bundle the
 * agent reads.
 * @param props
 * @param props.params
 */
export default async function DataRoomPage(props: { params: Promise<{ locale: string; id: string }> }) {
  ...
  const working = room.artifacts.filter(a => !documents.includes(a) && !logs.includes(a) && !filedSources.includes(a));
      ...
      >
        ...
        <Section eyebrow="Documents" data-testid="room-documents">
          {documents.length === 0
            ...
            : (
                <ListRows>
                  {documents.map(a => <DocumentRow key={a.id} a={a} href={artifactOpen(a)} />)}
                </ListRows>
              )}
        </Section>
