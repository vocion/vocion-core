...
describe('a key that cannot be read', () => {
  it('raises, so the MCP layer marks the result as an error', async () => {
    // `interfaces/mcp/server.ts` catches whatever a handler throws and answers
    // with `isError: true` and the message. Returning a tidy `{ error: … }`
    // object instead would reach a programmatic client as a *successful*
    // result, and a caller checking `isError` would treat a failed search as
    // ordinary output. The message is already safe to show — the error class
    // carries its own wording and none of the vault's.
    search.mockRejectedValueOnce(new ToolProviderKeyUnavailableError('tavily'));

    await expect(handlerFor('web_search')({ query: 'anything' }))
      .rejects
      .toBeInstanceOf(ToolProviderKeyUnavailableError);
  });

  it('carries a message with none of the vault\'s own words in it', async () => {
    search.mockRejectedValueOnce(new ToolProviderKeyUnavailableError('tavily'));

    await expect(handlerFor('web_search')({ query: 'anything' }))
      .rejects
      .toThrow(/^the workspace's stored tavily key could not be read$/);
  });
});
