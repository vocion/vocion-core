...
// AuthGuards pulls in next-auth, which does not import cleanly here; the
// action only needs the project row.
vi.mock('@/routers/AuthGuards', () => ({
  loadProject: async (id: string) => {
    const { db } = await import('@/libs/DB');
    const { projectSchema } = await import('@/models/Schema');
    const { eq } = await import('drizzle-orm');
    return (await db.select().from(projectSchema).where(eq(projectSchema.id, id)))[0] ?? null;
  },
}));
