 ...
 * as a Bearer header. Like every other connector here the token is resolved
 * per run from the encrypted vault by `SourceCredentialService`, against the
 * `notion` descriptor in `libs/platforms/registry.ts` — never from a
 * `process.env.NOTION_*` and never cached in a module-level client. Notion
 * scopes an integration by explicit sharing, so "what syncs" is decided in
 * Notion's UI by sharing pages with the integration — this connector
 * deliberately has no allow-list config to duplicate that decision badly.
 *
 ...
 * @param rich - The `rich_text` array from a block, property or title.
 */
...
/**
 * Fetch with Notion-appropriate failure handling: the shared helper honours the
 * `Retry-After` a 429 asked for (Notion rate-limits at roughly three requests a
 * second per integration), and 401/403 fail actionably — an unshared page or a
 * revoked token is never fixed by retrying.
 ...
 * @param blockId - The page or block whose children to read.
 * @param headers - Auth + version headers for the run.
 * @param budget - Per-document request budget, shared across the recursion.
 * @param budget.requests - Requests already spent on this document.
 * @param depth - How many levels deep this call already is.
 */
