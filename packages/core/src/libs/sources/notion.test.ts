...
describe('notionConnector', () => {
  ...
  it('keeps two orgs on their own keys, in sequence', async () => {
    // The token is resolved per run out of `ctx.credentials`, never read from
    // the environment and never cached in a module-level client. Two syncs in
    // a row therefore have to spend two different keys — a cache would send
    // the first org's token on the second org's call.
    const fetchMock = vi.fn(async () => res({ results: [], has_more: false, next_cursor: null }));
    vi.stubGlobal('fetch', fetchMock);

    await collect(notionConnector.sync(ctx({ orgId: 'org_first', credentials: { token: 'ntn_first' } })));
    await collect(notionConnector.sync(ctx({ orgId: 'org_second', credentials: { token: 'ntn_second' } })));
    ...
    expect(sent).toEqual(['Bearer ntn_first', 'Bearer ntn_second']);
  });

  it('authenticates through the `notion` credential platform, not an env var', async () => {
    // `platformForConnectorSlug` is the bridge SourceCredentialService walks to
    // find the vault row for a sync. Without a descriptor naming this slug the
    // connector would resolve no credentials at all and refuse every sync.
    const platform = platformForConnectorSlug('notion');

    expect(platform?.id).toBe('notion');
    expect(platform?.keySource).toBe('supplied');
    // The field name is the storage contract between the vault and the read of
    // `ctx.credentials.token` above.
    expect(platform?.fields.map(f => f.name)).toEqual(['token']);
  });
