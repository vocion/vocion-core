...
// (packages/core/templates/base/pack.yaml) ships at core@2.1.0 and is
...
describe('loadWorkspace — base pack (extends)', () => {
  ...
  it('bare `core` tracks the shipped version', () => {
    ...
    expect(loaded.pack?.manifest.version).toBe('2.1.0');
  });

  it('a matching version pin resolves', () => {
    const loaded = loadWorkspace(workspace('extends: core@2.1.0\n'));
  ...
  it('a pin that does not match the shipped version throws', () => {
    expect(() => loadWorkspace(workspace('extends: core@9.9.9\n')))
      .toThrow(/pins core@9\.9\.9 but the shipped pack is core@2\.1\.0/);
  });
