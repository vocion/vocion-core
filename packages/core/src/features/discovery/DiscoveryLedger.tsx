 ...
 * linked to Needs you). Dropped calls are rows here too — a call classified
...
/**
 * Where the human decision lives: the proposal's own row on Needs you when we
 * know the run (PR #348 folded Review into the inbox), otherwise the proposal
 * lane of the list.
 */
...
function Entry({ e }: { e: DiscoveryEntry }) {
  ...
  // What a person did with it: the review state, linked to Needs you where
  // they did (or will do) it. Absent when the router never asked anyone.
  const human = e.reviewStatus
    ? (
        <Link href={reviewHref(e)} className="underline decoration-border underline-offset-2 hover:text-foreground hover:decoration-foreground">
