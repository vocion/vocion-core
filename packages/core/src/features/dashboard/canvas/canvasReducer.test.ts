...
import { canvasReducer, COLUMNS, fillTileMessage, initialCanvasState, layoutTiles, MIN_EMPTY, MIN_SLOTS, placementOf } from './canvasReducer';
...
describe('canvasReducer', () => {
  ...
  it('always leaves at least two empty tiles, adding a row when the last row is full', () => {
    // 3 + 3 columns used exactly → without the rule, zero placeholders.
    const full = canvasReducer(initialCanvasState, { type: 'set', artifacts: [
      art(1, 'table', { slot: 0, span: 3 }),
      art(2, 'table', { slot: 1, span: 3 }),
      art(3, 'table', { slot: 2, span: 3 }),
    ] });
    const tiles = layoutTiles(full);
    const empties = tiles.filter(t => t.kind === 'empty');

    expect(empties.length).toBeGreaterThanOrEqual(MIN_EMPTY);
    expect(tiles.reduce((n, t) => n + t.span, 0) % COLUMNS).toBe(0);

    // After hiding one of two, placeholders are still there.
    let two = canvasReducer(initialCanvasState, { type: 'set', artifacts: [art(1, 'chart', { slot: 0, span: 2 }), art(2, 'record', { slot: 1, span: 1 })] });
    two = canvasReducer(two, { type: 'upsert', artifact: { ...art(2, 'record', { slot: 1, span: 1 }), pinned: false } });

    expect(layoutTiles(two).filter(t => t.kind === 'empty').length).toBeGreaterThanOrEqual(MIN_EMPTY);
  });
