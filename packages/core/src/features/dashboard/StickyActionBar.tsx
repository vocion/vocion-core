 ...
 *
 * Imported from PR #337 (review detail) into the pattern library with one
 * change: the feedback toggle's labels are props with English defaults rather
 * than `useTranslations('Review')`, because the bar also renders on surfaces
 * (the lead page, stories, the browser tests) that mount no intl provider.
 * Re-exported from `components/patterns`.
 */
...
export function StickyActionBar(props: {
  ...
  secondary?: BarAction[];
  field?: BarField;
  /** The collapsed field's toggle, when the page's wording differs. */
