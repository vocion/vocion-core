...
describe('readRollupDeclarations', () => {
  it('reads the software-factory plugin\'s declarations when the mounted workspace turns it on', async () => {
    ...
    expect(decls.filter(d => d.parentType === 'request').map(d => d.rollup.field).sort()).toEqual(['actualCents', 'estimateCents', 'reworkCents', 'reworkTaskCount', 'shippedAt', 'taskCount', 'varianceCents']);
