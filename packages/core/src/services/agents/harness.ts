...
async function buildGraph(orgId: string, agentSlug: string): Promise<CompiledAgentGraph> {
  ...
  const OUTPUT_DISCIPLINE = [
    ...
    'VOICE: write like a sharp human chief of staff texting a busy founder — not a chatbot. Hard bans: NO decorative or "stoplight" emoji (🔴🟡🟢✅) as bullets or status markers; NO templated scaffolding ("Here are your top three moves right now:", "I hope this helps", "Let me know if…"); NO filler closers ("Want me to draft all three now for your review?"). For a short ranked list do NOT wrap each item in its own ##/### header or --- rule — keep it tight (a bold lead-in + one line each). Lead with the move, be specific, cut hedging. Density over decoration.',
