...
describe('buildTeamViews (pure)', () => {
  ...
  const teamRow = (slug: string, name: string, leadAgentSlug: string | null, accountableUserId: string | null) =>
    ({ id: 1, orgId: ORG, projectId: null, slug, name, description: null, leadAgentSlug, accountableUserId, goal: null, kpis: [], measures: [], updatedAt: new Date(), createdAt: new Date() });
