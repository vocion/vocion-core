...
describe('taskKeyOf', () => {
  ...
  // Read off production on 2026-09-21: the harness sends the contract nested
  // under `task`, snake_case. Three of the thirteen failed runs that day were
  // one task, and only this shape finds them.
  it('reads the contract the harness actually sends: input.task.task_id', () => {
    const one = { agentSlug: 'send-engineer', input: { task: { task_id: 'send-0005-rename-stamp', request_id: 'stamp-rename', attempt: 1 }, record: {} } };
    const two = { agentSlug: 'send-engineer', input: { task: { task_id: 'send-0005-rename-stamp', request_id: 'stamp-rename', attempt: 2 } } };
    const other = { agentSlug: 'send-engineer', input: { task: { task_id: 'send-0006-e2e-intake-filter' } } };

    expect(taskKeyOf(one)).toBe(taskKeyOf(two));
    expect(taskKeyOf(one)).not.toBe(taskKeyOf(other));
    expect(taskKeyOf(one)).not.toBe(taskKeyOf({ agentSlug: 'send-engineer', input: {} }));
  });
});
