 ...
 * so this reads the keys the factory actually sets, in order, and falls back
 * to the agent — never to the run id, which would make every run its own task
 * and no third attempt would ever be found.
 * @param run
 */
export function taskKeyOf(run: Pick<FailedRun, 'agentSlug' | 'input'>): string {
  const input = run.input ?? {};
  for (const key of ['taskId', 'taskKey', 'contractId', 'engineeringTaskId', 'requestId'] as const) {
    const value = input[key];
    if (typeof value === 'string' && value.trim()) {
      return `${run.agentSlug}:${key}:${value.trim()}`;
    }
    if (typeof value === 'number' && Number.isFinite(value)) {
      return `${run.agentSlug}:${key}:${value}`;
    }
  }
