-- 0103 — scoped memory, expand step (plan: "Scoped agent memory on the
-- LangGraph Store", Phase 1). Creates the store and copies every live rule
-- into it; nothing is dropped here — 0104 is the contract step, so a reader
...
-- concurrent/0103. All copies are idempotent (ON CONFLICT DO NOTHING /
...
--> statement-breakpoint
-- The copies and backfills run only while the source tables still exist —
-- 0104 drops them, so a re-run (or a DB restored mid-sequence) is a no-op
-- here rather than an error.
DO $$
BEGIN
  IF to_regclass('public.learning_step') IS NOT NULL THEN
...
DO $$
BEGIN
  IF to_regclass('public.learning') IS NOT NULL THEN
