...
- **[NEW] Context compaction** — `pruneMessages` (AI SDK) / `compaction.thresholdPercent` (Eve). *Vocion:* **none** — `fetchUrl.ts` returns whole pages uncapped as of LARK-258, so there is no per-tool cap standing in for this either. *Payoff:* fixes long-run context bloat → cost/latency/quality collapse.
