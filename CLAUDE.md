...
> Conceptual explanation for humans — the loop vs the model vs the AWS account, and what AgentCore actually is — lives in `docs/agent-execution.md`. Keep the two in sync when this section changes.

The same deepagents loop also ships as a standalone artifact — **`packages/agent-runtime`** — with the BYOA HTTP contract (`POST /invocations` SSE + `GET /ping`), hostable on a laptop or AWS Bedrock AgentCore Runtime (same bundle). Two execution layers share one event contract, selected per agent via `harness.provider` in workspace YAML (`local` | `runtime`) or fleet-wide via `VOCION_AGENT_PROVIDER`. **Both run OUR loop** — `runtime` only moves it out of process, so AgentCore hosts the container rather than driving the agent. AWS's own managed harness (`provider: agentcore`, `CreateHarness`/`InvokeHarness`) is gone. It let AWS drive the loop, which cost us everything the loop implements: it exposed one tool (`search_knowledge`) against the full registry, with no subagents, playbooks or approval gates. **`Veerio-Life/veerio-vocion` ran `event-ingestion-lead` on it** — that agent has to move to `provider: runtime` (their AWS account needs `provision.sh` + `deploy-runtime.sh`, and `VOCION_AGENT_RUNTIME_ARN` + `VOCION_TOOL_ENDPOINT_URL` in their compose) before this lands, or their next `workspace:apply` fails validation on the narrowed enum.
...
- **Bedrock implies `runtime`**: an agent with `harness.modelProvider: bedrock` and NO `harness.provider` is dispatched to the artifact — `defaultHarnessProviderFor` in `AgentService.ts`. Choosing the vendor also chooses where the loop runs, since on a deployed installation the artifact IS AgentCore Runtime. `provider: local` next to it opts back out, `VOCION_AGENT_PROVIDER` still overrides fleet-wide, and `VOCION_DISABLE_RUNTIME=1` still forces the in-process loop. `harness.provider` is deliberately NOT defaulted in `libs/workspace/schemas.ts` — a stored `local` would make this default unreachable for every workspace-applied agent, so **agents applied before this change need one `workspace:apply` to pick it up**.
...
- **Cutover status**: `sales-assistant` runs on `provider: runtime` (workspace YAML). Dev therefore needs the artifact running — `npm run dev:agent-runtime` (:8080) — or set `VOCION_DISABLE_RUNTIME=1` to force the in-process loop.
