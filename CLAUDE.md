...
- Infra: `infra/agentcore/{provision,deploy-runtime,smoke-invoke}.sh` (ENV=dev, profile `metacto`, us-west-2). E2E: `src/scripts/smoke-runtime.ts` (`--provider` to force a specific provider). No CI deploy: core holds no AWS account, so the runtime is deployed by the parent project that owns the account, which calls these scripts from an operator machine or its own pipeline. See `packages/agent-runtime/README.md` and `docs/deployment/parent-project-pattern.md`.
